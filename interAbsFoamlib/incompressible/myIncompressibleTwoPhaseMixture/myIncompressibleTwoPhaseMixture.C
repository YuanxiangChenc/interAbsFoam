...
#include "myIncompressibleTwoPhaseMixture.H" 			//include new my.H
...
//-----------------------------Modification: constructor---------------
    ...
    D1_("D", dimensionSet(0, 2, -1, 0, 0, 0, 0), nuModel1_->viscosityProperties().lookup("D")),
    D2_("D", dimensionSet(0, 2, -1, 0, 0, 0, 0), nuModel2_->viscosityProperties().lookup("D")),
//-----------------------------end-------------------------------------
    U_(U),
    phi_(phi),
/////////////////////////////////////////////////////////////////////////
...
//--------------------modification:calculate thermal conducivity----------------
//------------------- define function kappaf() ---------------------------------
Foam::tmp<Foam::surfaceScalarField>
Foam::incompressibleTwoPhaseMixture::kappaf() const
{
    ...
    return tmp<surfaceScalarField>::New
    (
        "kappaf",			// volume mean, not mass mean. 
	(
              alpha1f * cp1_ * (1 / Pr1_)
            * fvc::interpolate(nuModel1_->nu()) * rho1_
            + (scalar(1) - alpha1f) * cp2_
            * (1 / Pr2_) * fvc::interpolate(nuModel2_->nu()) * rho2_
	) / (alpha1f + (scalar(1) - alpha1f) ) 
	
    );
}

Foam::tmp<Foam::surfaceScalarField>
Foam::incompressibleTwoPhaseMixture::Df() const
{
    const surfaceScalarField alpha1f
    (
        min(max(fvc::interpolate(alpha1_), scalar(0)), scalar(1))
    ...
    return tmp<surfaceScalarField>::New
    (
        "Df",			// volume mean, not mass mean.
	(
              alpha1f * D1_
            + (scalar(1) - alpha1f) * D2_
	) / (alpha1f + (scalar(1) - alpha1f)) 	
    );
}
//--------------------------------end--------------------------------------------
...
//------------------------------modification:read()---------------------------
            ...
            nuModel1_->viscosityProperties().readEntry("D", D1_);
            nuModel2_->viscosityProperties().readEntry("D", D2_);
//------------------------------end-------------------------------------------
            return true;
        }
    }
